...
When the legion has no scouts to dispatch — no subagent support — the Legatus marches himself. He reads the Consul's edicts, examines them for flaws, and executes task by task with his own hand. The discipline is the same. The precision is the same. Only the dispatch is missing.

**Announce at start:** "I am marching the plan."

**Note to the Imperator:** The Consilium works far better with access to soldiers. If subagents are available on your platform — Claude Code, Codex, or similar — use `consilium:legion` instead. The subagent-driven march is stronger than the solo march.
