...
## Contract Inventory And Spec Verification

This is the Codex-consumed Consul rule source for spec verification routing. Codex Consul follows this section even when running outside the Consilium repo cwd.

Specs that touch any canonical contract surface must include a Contract Inventory section or equivalent unambiguous label. The canonical six surfaces are: wire shape on a module boundary, API contract at a module boundary, idempotency anchor, link.create boundary, workflow ownership claim, subscriber boundary. Each Inventory entry names the surface with enough specificity to locate the corresponding contract definition elsewhere in the same spec. A spec that touches no contract surface still declares an empty Inventory with a one-line reason. Inventory is prose/list discipline, not metadata or a schema.

Spec verification has one announcement point:

> `Dispatching Tabularius, Censor, and Provocator for verification.`

If the Imperator says `skip` at that announcement, bypass all verification: no Tabularius, no Censor, no Provocator. If not skipped, dispatch `consilium-tabularius` in the foreground first using the active Consilium checkout's `claude/skills/references/verification/templates/contract-inventory-verification.md` template, or the same template path inside the installed Consilium Claude plugin when running from an installed runtime.

Handle Tabularius findings under verifier law: MISUNDERSTANDING halts and escalates. GAP is fixed inline and Tabularius is re-run before Censor plus Provocator. CONCERN is weighed on merit. SOUND proceeds. The re-dispatch cap follows the verification protocol default: max 2 iterations before escalation.

After Tabularius proceeds, dispatch Censor plus Provocator in parallel. Their missions are unchanged.
...
- Load the Tribune skill only when the runtime skill source resolves to the active Consilium checkout's `codex/skills/tribune`.
- If the runtime Tribune resolves to the active Consilium checkout's `claude/skills/tribune` or any other non-Codex path, treat Codex Tribune as unavailable and use the 14-field diagnosis packet inline from `$CONSILIUM_DOCS/doctrine/diagnosis-packet.md`.
