...
Dispatches independent verification on a diagnosis packet. Used by the `/tribune` skill after the Tribunus diagnosis stance writes the packet and self-reviews.
...
After the Tribunus diagnosis stance:
...
Agent tool:
  ...
  prompt: |
    ...
    Do NOT propose alternatives — that is the Tribunus diagnosis stance's or Provocator's role.
...
Agent tool:
  ...
  prompt: |
    ...
    1. Hypothesis extraction: the Tribunus diagnosis stance named one root cause. What are the plausible alternatives? Does the packet's evidence rule them out?
    2. Contrary evidence attack: the Tribunus diagnosis stance claims contrary evidence was searched for. What contrary evidence would you expect to find that the packet does not address?
    3. Fix-site attack: could the failure originate in a layer the Tribunus diagnosis stance did not consider? Name it.
...
3. Present the summary to the Imperator with attribution and the case-file path. Imperator approves → Tribunus diagnosis stance updates `Status: approved`, hands to Legion.
