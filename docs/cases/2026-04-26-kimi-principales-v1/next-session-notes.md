...
- **Per-call `input.timeout_ms` is accepted as input but not enforced per-request (Campaign-review GAP-B partial-fix).** The GAP-B fix wired `CONSILIUM_KIMI_TIMEOUT_MS` operator-level via the OpenAI SDK constructor (`timeout` option), which caps every request globally. Per-call `input.timeout_ms` is still validated and threaded through but not honored per-request — the SDK cap is the only ceiling. Integration case should either (i) extend the deps interface so `chatCompletionsCreate` accepts a per-call timeout (via `Promise.race` with `AbortController`, or by passing through the SDK's per-request `timeout` override) and update mocks accordingly, or (ii) drop `timeout_ms` from `VerifyLaneInput` if per-call enforcement is never wanted. The current state is functionally correct (no deadlock; SDK cap holds) but lies subtly to callers about the input field's effect.
- **`CONSILIUM_KIMI_TIMEOUT_MS` is parsed in two places.** GAP-B fix added defensive parsing in both `MoonshotClient.fromEnv` (for the SDK constructor) and `server.ts:startServer` (for the `timeout_ms` default in verify_lane input). They share the same default (45000) and the same defensive guard, but can drift if one is updated and not the other. Integration case can centralize via a shared env-loader module.
