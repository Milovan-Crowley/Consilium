# Consul Brief + Estimate-lite — Recon Discipline Before Speculators and Spec

**Status.** Ready for implementation after runtime-unification reconciliation.
**Date.** 2026-04-29.
**Target.** Consul role. Canonical source surfaces: `source/skills/claude/consul/SKILL.md` and `source/protocols/consul-routing.md`. Generated and compatibility surfaces are produced by `runtimes/scripts/generate.py`.
**Purpose.** Add two compact discipline checkpoints inside the Consul's existing flow: a Brief before speculator/retrieval dispatch, and an Estimate-lite before spec writing. The Brief prevents reconnaissance from becoming token theater. The Estimate-lite prevents the Consul from rushing into a spec when the request is actually multiple campaigns.

## Summary

Two checkpoints. No new agents. No new stages. No change to model routing, `/edicts`, `/legion`, `/march`, or visual companion.

- The **Consul Brief** is a compact framing the Consul writes before dispatching any speculator or retrieval/tracing fan-out. Speculators dispatch only against named unknowns from the Brief. Skippable for tiny / direct work.
...
- `source/skills/claude/consul/SKILL.md` Phase 1 currently flows: Domain knowledge → Codebase exploration (speculators) → Scope assessment → Medusa Rig. There is no checkpoint that gates speculator dispatch on a stated unknown.
- `source/skills/claude/consul/SKILL.md` Phase 3 currently flows: Spec Discipline Rule → Ambiguity elimination → present design → write spec → self-review → dispatch verification. There is no synthesis step between deliberation and spec writing that would surface multi-campaign scope.
- `source/protocols/consul-routing.md` describes the Codex-side Consul's default stance and dispatch rules but currently has no shaping checkpoint before retrieval/tracing fan-out.
...
The Consul's current Phase 1 lets speculators dispatch on vague intent. A speculator asked "what does the saved-product reorder flow look like?" without a stated unknown returns a broad survey. The Consul absorbs the survey, the Imperator's context window absorbs the Consul's absorption, and the spec ends up shaped by reconnaissance volume rather than reconnaissance precision.
...
- The Consul writes a compact Brief before any speculator or retrieval/tracing dispatch and uses the Brief's named unknowns as the dispatch trigger.
- Speculators dispatch only against unknowns that materially affect the spec or shorten the critical path.
...
The Brief is a compact framing of the work before any speculator, retrieval, or tracing dispatch is made.
...
|Unknowns|Facts the Consul does not yet know that materially affect the spec or critical path. The list that gates speculator/retrieval dispatch.|
|Likely code surfaces|Repos, modules, components, routes, or workflows the work is likely to touch. Best-effort, revisable.|
|Recon lanes|Which retrieval or speculator lanes are needed (none / domain interpretation / codebase tracing / docs / hybrid). Empty if no speculators will dispatch.|
|Decision gates|Questions that require Imperator or Consul judgment, not retrieval.|

**When required.** The Brief is required by default before the Consul dispatches any speculator/retrieval work, and required before the Consul writes a spec when the work touches more than one module, more than one repo, or names a domain concept the Consul has not just verified in this session.

**When skippable.** The Brief may be skipped only when **all** of the following hold:

- No speculator/retrieval dispatch is anticipated.
...
**Ordering.** The Brief precedes speculator/retrieval dispatch. It must be visible in the Consul's message thread before the message that dispatches speculators. It does not have to precede Phase 2 deliberation — when the Imperator's initial framing is too vague to articulate a Goal, the Consul may open deliberation first, extract enough clarity, and then write the Brief before dispatching retrieval.

**Presentation contract.** The Brief is presented inline in the Consul's message thread. The Imperator may interject. The Consul does not pause for explicit confirmation — the discipline is transparency, not approval gating. The Consul may amend the Brief mid-session as new facts surface; an amended Brief is presented again before any further speculator/retrieval dispatch. Speculators already in flight are re-judged against the amended Brief: if the unknown a speculator was dispatched against has been removed or reframed as a Decision gate, the Consul evaluates the return on relevance and may discard it as no longer load-bearing on the spec.

**Empty fields are valid.** A Brief with zero Unknowns, zero Domain concepts, or zero Recon lanes is acceptable. The Brief's value is the framing exercise as a whole, not the gating-on-unknowns mechanic alone — Goal, Non-goals, Known constraints, Likely code surfaces, and Decision gates discipline the spec even when retrieval is not needed.

### 2. Speculator Dispatch Gating

A speculator, retrieval, or tracing dispatch must reference a named unknown from the current Brief. The dispatch prompt must state the unknown and how its resolution materially affects the spec or the critical path.

If the unknown cannot be resolved by any available rank, the unknown is escalated to the Imperator as a decision gate, not dispatched.

More speculators is not automatically better. A speculator that resolves no Brief unknown is not dispatched.
...
|Forces|Which dispatch ranks the Consul anticipates the work will need (speculators, interpres, arbiter, verifier ranks, implementer ranks), with reasoning. Informational only — does not override existing routing rules and does not pick models.|
...
The skip linkage is asymmetric by design. The Estimate-lite serves two functions: decomposition detection AND framing rigor (forcing the Consul to articulate Intent, Effects, Terrain, Forces, Coordination, Control before spec writing). Brief-skipped under tiny / direct implies the work is small enough that framing rigor is not load-bearing. Brief-required for any reason — domain concept, cross-module scope, speculator dispatch — implies the framing rigor is, even when decomposition risk is low.

**Ordering.** The Estimate-lite is written after deliberation produces a coherent design — including any speculator returns integrated into the Consul's understanding of Effects, Terrain, and Forces — and before the spec is written. It is presented inline before the spec write.

**Forces field — non-routing clause.** The Forces field documents which ranks the Consul anticipates the work will need. It is informational synthesis. It does not override the existing dispatch rules in `source/skills/claude/consul/SKILL.md`, `source/protocols/consul-routing.md`, or any verification-template dispatch contract. It does not select a model or a reasoning policy. If Forces appears to require a rank that does not exist or a routing change, the Consul names that as a Decision gate, not a routing override.
...
|Active dispatch agents and ranks (`consilium-{censor,praetor,provocator,tribunus,speculator-*,interpres-*,arbiter,centurio-*}` plus Consul/Legatus surfaces)|Unchanged. No new agent files.|
|Generated and compatibility surfaces (`claude/skills`, `codex/source`, `codex/agents`, `generated/*`)|Derived only from `runtimes/scripts/generate.py`; do not hand-edit generated copies.|
|Doctrine files (`$CONSILIUM_DOCS/doctrine/`)|Unchanged.|

The plan implementing this spec edits the Consul discipline prose in canonical source, adds Codex-side parity in canonical source, then runs generation. It does not edit machinery elsewhere. Generated diffs are expected only as derived output and may not change dispatch rules beyond carrying the new shaping prose.
...
|Brief precedes speculator dispatch|High|Imperator directive required the checkpoint before retrieval dispatch; runtime-unification vocabulary names that rank Speculator.|
|Speculator-dispatch gating on named Brief unknowns|High|Imperator directive required dispatch only for named unknowns that materially affect the spec or critical path; runtime-unification vocabulary names that rank Speculator.|
...
- The Consul's Phase 1 has a named Brief checkpoint between Scope assessment and any speculator dispatch.
- The Consul's Phase 3 has a named Estimate-lite checkpoint between deliberation and spec writing.
- A speculator dispatch prompt that does not reference a Brief unknown is recognizable as a discipline violation by reading the Consul's own message thread.
...
- After implementation, hand-edited files are limited to `source/skills/claude/consul/SKILL.md` and `source/protocols/consul-routing.md`; any other changed runtime files are generated compatibility output from `runtimes/scripts/generate.py`.
...
Imperator: "Add a `--dry-run` flag to the existing `case-new` script." All four skip conditions hold (single file, no speculator dispatch, no domain concept, success metric is one sentence). The Consul skips the Brief, skips the Estimate-lite, proceeds to the spec. The transcript names the skip and the four conditions in one line.

**Scenario B — normal task with one unknown.**
Imperator: "Let customers rename their saved products." Scope assessment shows one campaign but the saved-product domain concept must be re-verified and the cart line-item display surface is unknown. The Consul writes the Brief naming both unknowns. A single speculator dispatches against the cart line-item unknown. The doctrine read covers the saved-product concept. After deliberation, the Consul writes the Estimate-lite with all six sections. No decomposition signal. The Consul writes the spec.

**Scenario C — multi-campaign request disguised as one.**
Imperator: "Let customers rename and reorder saved products." Scope assessment surfaces two distinct outcomes. The Consul writes the Brief. Two speculators dispatch against two distinct unknowns. The Estimate-lite shows two independent Effects sets sharing only the saved-product Terrain — rename and reorder share no domain invariant, no anchor, no module beyond the saved-product table itself. Decomposition signal triggers (criterion #1 and the semantic reading of criterion #4). The Consul halts, announces the signal, asks the Imperator to decompose or confirm. The Consul does not write a spec until the Imperator decides. If the Imperator confirms a single combined campaign, the Consul records the trigger and the override in the spec before proceeding.

## Open Decisions Deferred to Plan

These are HOW questions and belong to the plan author, not this spec:

- Exact insertion point and prose wording inside `source/skills/claude/consul/SKILL.md`.
- Whether to mirror the discipline into `source/protocols/consul-routing.md`. The discipline applies to the Consul role; whether the Codex runtime gets an explicit prose insertion is a plan judgment, bounded by the negative-scope contract above.
...
- Whether the implementation also adjusts the Phase 1 prose order so that Scope assessment visibly precedes Codebase exploration (the live canonical SKILL.md describes them in the reverse order; that prose ordering is a plan-level fix).
