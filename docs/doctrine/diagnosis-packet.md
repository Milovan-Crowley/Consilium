...
The packet is the Tribunus diagnosis stance's core artifact. Written during Phase 4 (Packet construction), verified by the Tribunus (diagnosis stance) + Provocator in Phase 5, persisted to the case file in Phase 6 before the Imperator gate.
...
4. **Files/routes inspected** — absolute paths the speculators hit, with short annotation of what was inspected.
...
6. **Root-cause hypothesis** — the Tribunus diagnosis stance's single leading explanation. One paragraph. Not a menu of possibilities.
...
11. **Fix threshold** — `small` | `medium` | `large` | `contain` per `fix-thresholds.md`. The Tribunus diagnosis stance proposes; the Imperator overrides at invocation if needed.
...
13. **Open uncertainty** — what the Tribunus diagnosis stance does NOT know but the fix depends on. Intentional honesty. Becomes the Provocator's target — not a weakness but a flag for adversarial review.
...
- A field with placeholder syntax (`[placeholder marker]`, `[placeholder marker]`, `...`) is a GAP.
