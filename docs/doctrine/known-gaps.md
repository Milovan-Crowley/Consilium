...
Product-specific recurring bug memory. Read directly from `$CONSILIUM_DOCS/doctrine/known-gaps.md` by the Tribunus diagnosis stance on every `/tribune` session, by the Consul during reconnaissance, and by any verifier that receives known-gap context. Each entry is a hypothesis accelerator with strict discipline rules.
...
- **Route:** Tribunus diagnosis stance + cross-repo-debugging guide + `medusa-dev:building-with-medusa` → (only after idempotency/ownership/price-unit boundaries explicit) Legatus → Centurio + `medusa-dev:building-with-medusa`.
...
- **Route:** Tribunus diagnosis stance (UI-permission vs backend-enforcement disagreement) → Legatus → Centurio + `medusa-dev:building-storefronts` (for explaining user-flow split between `/accept-invite` and in-app onboarding).
...
- **Route:** Tribunus diagnosis stance → (if backend propagation gap) Legatus → Centurio + `medusa-dev:building-with-medusa` → (if rendering gap) Legatus → Centurio + `medusa-dev:building-storefronts`.
...
- **Route:** Tribunus diagnosis stance + cross-repo-debugging guide + `medusa-dev:building-with-medusa` → (after migration direction explicit) Legatus → Centurio + `medusa-dev:building-with-medusa`.
...
- **Route:** Tribunus diagnosis stance (frontend-vs-backend mutation acceptance) | Tribunus diagnosis stance + cross-repo-debugging guide + `medusa-dev:building-with-medusa` (backend `hasPermission` enforcement).
...
- **Route:** Tribunus diagnosis stance (status contract mismatches) + Tribunus diagnosis stance + cross-repo-debugging guide + `medusa-dev:building-with-medusa` (transition guards and call sites).
...
- **Route:** Tribunus diagnosis stance (storefront + backend routes + metadata) → (fall back to) Tribunus diagnosis stance + cross-repo-debugging guide + `medusa-dev:building-with-medusa` when failure localizes to backend route metadata mutation.
